...
export class CameraService {
  ...
  private extractPriceFromText(text: string): string[] {
    console.debug('extraindo preço...', text);
    ...
    if (priceMatches && priceMatches.length > 0) {
      ...
      console.debug('Preços encontrados:', uniquePrices);
      return uniquePrices;
    }
  ...
  async scan_preco(
    ...
    return new Promise(async (resolve, reject) => {
      ...
      console.debug('processando video...', video);
      ...
      if (ctx) {
        console.debug('desenhando a imagem no canvas...');
        ...
        console.debug('carregando worker para leitura...');
        await worker.load();
